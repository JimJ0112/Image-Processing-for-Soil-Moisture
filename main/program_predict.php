...
function hsv_predictions()
{
    ...
    foreach ($dn_values as $val) {
        ...
        if ($set_type === "testing_set") {
            $testing_set_array_assoc[$second_iterator] =  $val;
            $testing_set_array[$second_iterator] =  array($val->H, $val->S, $val->V);
            $testing_set_labels[$second_iterator] = $val->mc;
            $testing_set_Images[$second_iterator] = $val->image;
        } else if ($set_type === "training_set") {
            $training_set_array_assoc[$second_iterator] =  $val;
            $training_set_array[$second_iterator] =  array($val->H, $val->S, $val->V);
            $training_set_labels[$second_iterator] = $val->mc;
            $training_set_Images[$second_iterator] = $val->image;
        }
...
function YCbCr_predictions()
{
    ...
    foreach ($dn_values as $val) {
        ...
        if ($set_type === "testing_set") {
            $testing_set_array_assoc[$second_iterator] =  $val;
            $testing_set_array[$second_iterator] =  array($val->Y, $val->Cb, $val->Cr);
            $testing_set_labels[$second_iterator] = $val->mc;
            $testing_set_Images[$second_iterator] = $val->image;
        } else if ($set_type === "training_set") {
            $training_set_array_assoc[$second_iterator] =  $val;
            $training_set_array[$second_iterator] =  array($val->Y, $val->Cb, $val->Cr);
            $training_set_labels[$second_iterator] = $val->mc;
            $training_set_Images[$second_iterator] = $val->image;
        }
    ...
    echo ' <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">';
    $new_iterator_training = 0;
    echo "<h3 class='m-auto w-100 text-center mt-2'> From YCbCr color space </h3> <br/> <hr/> ";
